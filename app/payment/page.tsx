...
export default function PaymentPage() {
  ...
  const handleScreenshotUpload = async () => {
    ...
    // Compress both files before uploading
    const [compressedScreenshot, compressedIdCard] = await Promise.all([
      compressImage(screenshot),
      compressImage(idCard),
    ])

    const screenshotExt = compressedScreenshot.type === 'application/pdf'
    ...
    const { error: screenshotError } = await supabase.storage
      .from('Abstracts')
      .upload(screenshotPath, compressedScreenshot, { upsert: true })
    ...
    const idCardExt = compressedIdCard.type === 'application/pdf'
      ? idCard.name.split('.').pop()
      : 'jpg'
    const idCardPath = `id-cards/${user.id}.${idCardExt}`

    const { error: idCardError } = await supabase.storage
      .from('Abstracts')
      .upload(idCardPath, compressedIdCard, { upsert: true })
        ...
        <div style={{ background: '#fff', border: '1px solid #d4e8cc', borderRadius: '16px', padding: '2.5rem', width: '100%', maxWidth: '480px' }}>
          ...
          {method === 'manual' && (
            <div>
              ...
              >
                {screenshot ? (
                  <div>
                    ...
                    <p style={{ fontSize: '11px', color: '#6b8c6b', marginTop: '4px' }}>
                      {(screenshot.size / 1024).toFixed(0)}kb → will be compressed before upload
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <p style={{ fontSize: '12px', color: '#6b8c6b', marginTop: '4px' }}>JPG, PNG or PDF · Any size</p>
                  </>
                )}
              </div>
              <input
              ...
              >
                {idCard ? (
                  <div>
                    ...
                    <p style={{ fontSize: '11px', color: '#6b8c6b', marginTop: '4px' }}>
                      {(idCard.size / 1024).toFixed(0)}kb → will be compressed before upload
                    </p>
                  </div>
                ) : (
                  <>
              ...
              >
                {uploading ? 'Compressing & uploading... ⏳' : 'Submit Payment & ID Card →'}
              </button>
